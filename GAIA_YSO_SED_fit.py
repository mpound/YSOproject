...
select_alpha = [-10,-2]
select_class = "Class_III"
select_class = ""

infile = "YSOcWithAllPhotometry.votab"
...
outdir="fits_GAIA_YSOc_"+select_class+"/"
...
photplotdir="plots_GAIA_YSOc_photometry/"
...
#            validity = 0
    ...
    if not abad: 
        ...
        if do_plot:
            ...
            plt.legend(handles=patches,loc='best',numpoints=1,ncol=1, framealpha=0.5) #bbox_to_anchor=(.9,0.65),framealpha=0.25)
...
default_av_range=[0., 10.]
dust_model = 'whitney.r550.par'
topdir        = '/n/subaruraid/mpound/'
model_dir     = topdir+'sedfittermodels/models_r17/'
...
if do_fit:
    for s in seds[0:1]:
        ...
        select_format=('N',20)
        for m in models:
            sed_model_dir = model_dir+m
            outfit = outdir+"/"+nospacename+'_'+m+'.sedfit'
            outplot = plotdir+nospacename+'_'+m+"/"
            try:
            ...
            if thisbad:
                ...
                write_parameters(outfit,outdir+"/"+nospacename+"_"+m+"_parameters.txt",select_format=select_format)
                write_parameter_ranges(outfit,outdir+"/"+nospacename+"_"+m+"_parameters_ranges.txt",select_format=select_format)
                plot(input_fits=outfit, output_dir=outplot,format='png',
                         plot_mode='A',plot_name=True,
                         select_format=select_format,
                         show_convolved=False, show_sed=True, modstr=m)
